...
def get_planet_lon(jd: float, planet: int) -> float:
    ...
    if planet == -1:
        # Ketu = Mean Rahu + 180° — use MEAN_NODE to match Drik Panchang
        rahu_trop = swe.calc_ut(jd, swe.MEAN_NODE, _CALC_FLAGS)[0][0]
...
def get_full_panchang(target_date: date, lat: float, lon: float,
                      time_h: int = None, time_m: int = 0) -> Dict:
    """Complete panchang for a date at given coordinates.

    time_h / time_m (optional local time): when provided, planet positions are
    computed at that exact local time instead of live-now (today) or noon (other dates).
    Tithi, nakshatra, yoga, karana and muhurtas always use the sunrise JD.
    """
    ...
    # Planet JD: explicit time > live-now (today) > noon (other dates).
    # Round to 5 decimal places (~0.86s precision) for stable lru_cache keys.
    if time_h is not None:
        _dt_time = datetime(target_date.year, target_date.month, target_date.day,
                            int(time_h), int(time_m), 0)
        _planet_jd = round(datetime_to_jd(_dt_time, lat, lon), 5)
    elif target_date == date.today():
        _now_dt = datetime.utcnow()
        _planet_jd = round(swe.julday(_now_dt.year, _now_dt.month, _now_dt.day,
                                      _now_dt.hour + _now_dt.minute / 60.0
                                      + _now_dt.second / 3600.0), 5)
    else:
        _planet_jd = round(jd_noon, 5)
    planets = {}
    for name, planet_id in PLANETS.items():
        p_lon = get_planet_lon(_planet_jd, planet_id)
...
def match_36_gunas(boy_nak: str, girl_nak: str,
                   boy_gotra: str = "", girl_gotra: str = "",
                   boy_moon_lon: float = None, girl_moon_lon: float = None) -> Dict:
    """Ashtakoot Guna Milana per Brihat Parashara Hora Shastra (BPHS).

    Kutas and max points (total = 36):
      1. Varna        (1)
      2. Vashya       (2)
      3. Tara / Dina  (3)
      4. Yoni         (4)
      5. Graha Maitri (5)
      6. Gana         (6)
      7. Bhakoot / Rashikoot (7) — Moon-longitude rashi when available
      8. Nadi         (8)

    Bhakoot uses Moon longitude (degree-based rashi) when boy_moon_lon /
    girl_moon_lon are provided, falling back to NAKSHATRA_RASHI table.
    All other kutas remain nakshatra-based as BPHS prescribes.
    """
    results = {}
    total   = 0

    # Rashi indices from nakshatra table — used by all kutas except Bhakoot
    b_rashi = NAKSHATRA_RASHI[boy_nak]
    g_rashi = NAKSHATRA_RASHI[girl_nak]

    # ── 1. Varna (1 point) ────────────────────────────────────────────────────
    # BPHS: groom's varna must be equal to or higher than bride's.
    # Order (high→low): Brahmin > Kshatriya > Vaishya > Shudra
    # Mleccha treated as below Shudra (some texts omit; kept for completeness).
    ...
    # ── 2. Vashya (2 points) ──────────────────────────────────────────────────
    # BPHS: same vashya = 2, one controls the other = 1, no relation = 0.
    # Control pairs per BPHS: Manava controls Chatushpada & Vanachara;
    # Jalchara controls Keeta; Chatushpada controls Vanachara.
    b_vashya = calc_vashya(b_rashi)
    g_vashya = calc_vashya(g_rashi)
    _vashya_controls = {
        "Manava":      {"Chatushpada", "Vanachara"},
        "Jalchara":    {"Keeta"},
        "Chatushpada": {"Vanachara"},
    }
    if b_vashya == g_vashya:
        vashya_pts, vashya_status = 2, "Full (same vashya)"
    elif (g_vashya in _vashya_controls.get(b_vashya, set()) or
          b_vashya in _vashya_controls.get(g_vashya, set())):
        vashya_pts, vashya_status = 1, "Partial (control)"
    else:
    ...
    # BPHS: count girl's nakshatra from boy's (inclusive), divide by 9,
    # take remainder (1-9). Odd tara (1,3,5,7,9) = auspicious for bride;
    # even tara (2,4,6,8) = inauspicious. Full score when both directions
    # (boy→girl AND girl→boy) are auspicious; partial if one; zero if both bad.
    b_idx = nakshatra_to_idx(boy_nak)
    g_idx = nakshatra_to_idx(girl_nak)
    bg_tara = ((g_idx - b_idx) % 27) % 9 + 1   # boy→girl tara (1-9)
    gb_tara = ((b_idx - g_idx) % 27) % 9 + 1   # girl→boy tara (1-9)
    _auspicious_tara = {1, 3, 5, 7, 9}
    bg_ok = bg_tara in _auspicious_tara
    gb_ok = gb_tara in _auspicious_tara
    if bg_ok and gb_ok:
        tara_pts, tara_status = 3, f"Auspicious (Tara {bg_tara}/{gb_tara})"
    elif bg_ok or gb_ok:
        tara_pts, tara_status = 1, f"Partial (Tara {bg_tara}/{gb_tara})"
    else:
        tara_pts, tara_status = 0, f"Inauspicious (Tara {bg_tara}/{gb_tara})"
    results['tara'] = {'points': tara_pts, 'max': 3, 'status': tara_status}
    total += tara_pts

    # ── 4. Yoni (4 points) ────────────────────────────────────────────────────
    # BPHS 4-tier: same animal + opposite gender = 4 (Uttama);
    # same animal + same gender = 3 (Madhyama);
    # friendly animals = 2; neutral = 1; hostile = 0.
    b_yoni, b_ygender = NAKSHATRA_YONI[boy_nak]
    g_yoni, g_ygender = NAKSHATRA_YONI[girl_nak]
    _friendly_yoni = {
        frozenset({"Horse",    "Elephant"}),
        frozenset({"Goat",     "Cow"}),
        frozenset({"Dog",      "Serpent"}),
        frozenset({"Cat",      "Rat"}),
        frozenset({"Deer",     "Monkey"}),
        frozenset({"Buffalo",  "Tiger"}),
        frozenset({"Lion",     "Mongoose"}),
    }
    if b_yoni == g_yoni and b_ygender != g_ygender:
        yoni_pts, yoni_status = 4, "Uttama (same animal, opposite gender)"
    elif b_yoni == g_yoni:
        yoni_pts, yoni_status = 3, "Madhyama (same animal, same gender)"
    elif (b_yoni, g_yoni) in HOSTILE_YONI:
        yoni_pts, yoni_status = 0, "Hostile"
    elif frozenset({b_yoni, g_yoni}) in _friendly_yoni:
        yoni_pts, yoni_status = 2, "Friendly"
    else:
        yoni_pts, yoni_status = 1, "Neutral"
    ...
    # BPHS scoring table (by combined relation of both lords):
    #   Both friends         = 5
    #   Friend + Neutral     = 4
    #   Both neutrals        = 3
    #   Friend + Enemy       = 1
    #   Neutral + Enemy      = 0 (BPHS gives 0, not 1)
    #   Both enemies         = 0
    # Same lord              = 5
    b_lord = RASHI_LORDS[b_rashi]
    g_lord = RASHI_LORDS[g_rashi]
    b_rel  = _graha_relation(b_lord, g_lord)
    g_rel  = _graha_relation(g_lord, b_lord)
    if b_lord == g_lord:
        gm_pts, gm_status = 5, "Same lord"
    elif b_rel == "friend" and g_rel == "friend":
        gm_pts, gm_status = 5, "Mutual friends"
    elif (b_rel == "friend" and g_rel == "neutral") or (b_rel == "neutral" and g_rel == "friend"):
        gm_pts, gm_status = 4, "Friend + Neutral"
    elif b_rel == "neutral" and g_rel == "neutral":
        gm_pts, gm_status = 3, "Mutual neutral"
    elif (b_rel == "friend" and g_rel == "enemy") or (b_rel == "enemy" and g_rel == "friend"):
        gm_pts, gm_status = 1, "Friend + Enemy"
    else:
        # neutral+enemy or enemy+enemy
        gm_pts, gm_status = 0, f"{b_rel.title()} + {g_rel.title()}"
    results['graha_maitri'] = {'points': gm_pts, 'max': 5,
                                'boy': f"{b_lord} (lord of {RASHI_NAMES[b_rashi]})",
                                'girl': f"{g_lord} (lord of {RASHI_NAMES[g_rashi]})",
                                'status': gm_status}
    total += gm_pts

    # ── 6. Gana (6 points) ────────────────────────────────────────────────────
    # BPHS matrix (boy gana, girl gana):
    ...
    gana_pts = _gana_pts.get((b_gana, g_gana), 0)
    ...
    # ── 7. Bhakoot / Rashikoot (7 points) ────────────────────────────────────
    # BPHS: uses Moon rashi (Janma Rashi) of both.
    # Degree-based rashi from Moon longitude is used when available (most precise);
    # falls back to nakshatra-table rashi otherwise.
    #
    # Interval = count from boy's rashi to girl's rashi (inclusive, 1-based).
    # 1-based interval k means (k-1) in 0-based difference.
    # Inauspicious 1-based pairs per BPHS: (2,12), (5,9), (6,8).
    # In 0-based forward diff d = (g_rashi - b_rashi) % 12:
    #   (2,12) → d=1 or d=11
    #   (5,9)  → d=4 or d=8
    #   (6,8)  → d=5 or d=7
    # 0 (same rashi) = 7 points (auspicious per BPHS).
    if boy_moon_lon is not None and girl_moon_lon is not None:
        b_rashi_bh = int(boy_moon_lon  % 360.0 / 30.0)
        g_rashi_bh = int(girl_moon_lon % 360.0 / 30.0)
        bhakoot_method = "Moon longitude"
    else:
        b_rashi_bh = b_rashi
        g_rashi_bh = g_rashi
        bhakoot_method = "Nakshatra table"
    d = (g_rashi_bh - b_rashi_bh) % 12   # 0-based forward diff (0-11)
    # Inauspicious 0-based diffs: 1,4,5,7,8,11
    _bhakoot_dosha = {1, 4, 5, 7, 8, 11}
    if d in _bhakoot_dosha:
        bhakoot_pts  = 0
        # Map d back to the 1-based interval pair for status message
        d1 = d + 1
        d2 = 12 - d
        bhakoot_status = f"Dosha ({d1}/{d2} interval)"
    else:
        bhakoot_pts    = 7
    ...
    results['bhakoot'] = {
        'points':       bhakoot_pts,
        'max':          7,
        'boy':          RASHI_NAMES[b_rashi_bh],
        'girl':         RASHI_NAMES[g_rashi_bh],
        'rashi_diff':   d,
        'rashi_method': bhakoot_method,
        'status':       bhakoot_status,
    }
    total += bhakoot_pts

    # ── 8. Nadi (8 points) ────────────────────────────────────────────────────
    # BPHS: different nadi = 8; same nadi = 0 (Nadi Dosha — serious defect).
    ...
    # ── Gotra check (informational, not a BPHS kuta) ──────────────────────────
    gotra_ok   = True
    gotra_note = ""
    if boy_gotra and girl_gotra:
        ...
        gotra_note = (" Different Gotras — Compatible" if gotra_ok
                      else " Same Gotra — Marriage not recommended per Dharmashastra")
