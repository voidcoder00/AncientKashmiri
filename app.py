...
with tabs[0]:
    ...
    with home_r:
        ...
        try:
            ...
            """, unsafe_allow_html=True)
            # Today's panchang festivals + Jantri events
            _today_fests   = _FEST_LOOKUP.get((home_p['masa'], home_p['paksha'], home_p['tithi_num']), [])
            _today_jantri  = get_jantri_events(today)
            _all_home_evts = _today_fests + _today_jantri
            if _all_home_evts:
                _fest_html = "".join(
                    ...
                    for fn, fc in _all_home_evts
                )
                st.markdown(
                    f'<div style="text-align:center;margin-top:6px">{_fest_html}</div>',
                    unsafe_allow_html=True
                )
            # Today's Jantri events — full list below card
            if _today_jantri:
                _jantri_rows = "".join(
                    f'<div style="display:flex;align-items:center;gap:8px;padding:4px 0;'
                    f'border-bottom:1px solid rgba(184,134,42,.1)">'
                    f'<span style="width:8px;height:8px;border-radius:50%;'
                    f'background:{fc};flex-shrink:0"></span>'
                    f'<span style="font-size:11px;color:var(--ink);line-height:1.4">{fn}</span>'
                    f'</div>'
                    for fn, fc in _today_jantri
                )
                st.markdown(
                    ...
                    f'color:var(--saffron);margin-bottom:6px">TODAY\'S EVENTS · आज के कार्यक्रम</div>'
                    f'{_jantri_rows}</div>',
                    unsafe_allow_html=True
                )
        except Exception:
            ...
            for week in cal_obj:
                ...
                for day in week:
                    if day == 0:
                        ...
                        tip_attr = ""
                        if evts:
                            tip_txt = "|".join(e for e, _ in evts)
                            # escape quotes for data attribute
                            tip_txt = tip_txt.replace('"', "&quot;")
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .day-cell {{
    ...
    min-width:26px; position:relative;
  }}
  ...
  .dot {{ display:inline-block; width:5px; height:5px;
...
<script>
  ...
  (function(){{
    ...
    if(cards.length > {_cur_month_idx}) {{
      var card = cards[{_cur_month_idx}];
      track.scrollLeft = card.offsetLeft - 12;
    }}
  }})();
  // Tooltip
