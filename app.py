...
with tabs[1]:
    ...
    if st.session_state.get("pan_show_fullcal", False):
        ...
        # Build HTML for each month card — compact grid with dots + hover tooltip
            ...
            for week in cal_obj:
                ...
                for day in week:
                    if day == 0:
                        ...
                        tip_attr = ""
                        if evts:
                            tip_txt = "|".join(e for e, _ in evts).replace('"', "&quot;")
                            tip_attr = f' data-tip="{tip_txt}"'
                        rows_html += (
                            f'<td><div class="{cls}"{tip_attr}>'
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  td {{ padding:2px; }}
  .day-cell {{
    border-radius:5px; padding:3px 2px; text-align:center; min-width:26px;
  }}
  .day-cell.has-evt {{ cursor:pointer; background:rgba(184,134,42,.12); }}
  .day-cell.is-today {{ background:#8B1C30 !important; }}
  .day-num {{ font-family:'Cinzel',serif; font-size:11px; line-height:1; }}
  .dots {{ text-align:center; margin-top:1px; }}
  .dot {{ display:inline-block; width:5px; height:5px; border-radius:50%; margin:0 1px; }}
  .tip {{
    display:none; position:fixed; z-index:9999;
    background:#1E0F06; border:1px solid #B8862A;
    color:#FDF8F0; font-size:10px; line-height:1.6;
...
<div class="hint">Hover a date to see events &nbsp;·&nbsp; Coloured dots = KP Jantri events &nbsp;·&nbsp; &#9632; = today</div>
...
<script>
  ...
  document.querySelectorAll('[data-tip]').forEach(function(el) {{
    ...
    el.addEventListener('mouseleave', function() {{ tip.style.display = 'none'; }});
        ...
        _stc.html(full_cal_html, height=420, scrolling=False)
    ...
    try:
        ...
        with right_col:
            ...
            wd_cells = "".join(
                ...
                f'padding:8px 4px;text-align:center;background:var(--cream2);'
            ...
            cells = [
                ...
                f'<div style="min-height:80px;background:transparent;'
            ...
            for dn in range(1, last_day.day + 1):
                ...
                # Nakshatra — small italic line
                nak_html = ""
                if nakshatra:
                    nak_c = "#DEB85A" if is_sel else "rgba(106,72,40,.7)"
                    nak_html = (
                        f'<div style="font-size:7.5px;color:{nak_c};font-style:italic;'
                ...
                evt_lines = ""
                # Show up to 3 events total, panchang events take priority
                shown = 0
                ...
                for fn, fc in jantri:
                    ...
                    evt_lines += (
                        ...
                        f'<span style="width:5px;height:5px;border-radius:50%;background:{fc_use};'
                        f'flex-shrink:0;margin-top:2px"></span>'
                        f'<span style="font-size:7.5px;color:{fc_use};'
                        f'overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'
                        f'max-width:100%">{fn}</span></div>'
                    )
                    shown += 1
                ...
                if total_evts > 3:
                    ...
                    evt_lines += (
                        f'<div style="font-size:7px;color:{more_c};margin-top:2px;'
                        f'font-style:italic">+{total_evts - 3} more</div>'
                    )

                cells.append(
                    f'<td style="padding:3px;vertical-align:top">'
                    f'<div style="background:{bg};border:{brd};border-radius:8px;{shd}'
                    f'padding:6px 5px 5px;min-height:80px;transition:background .12s">'
                    # date number + tithi
                    f'<div style="display:flex;align-items:baseline;gap:4px;margin-bottom:1px">'
                    f'<span style="font-family:Cinzel,serif;font-size:13px;font-weight:600;'
                    f'color:{nc};line-height:1">{dn}</span>'
                    f'<span style="font-size:8px;color:{tc};letter-spacing:.2px">{ts}</span>'
                    f'</div>'
                    # nakshatra
                    f'{nak_html}'
                    # events
            ...
            cells += [
                f'<td style="padding:3px;vertical-align:top">'
                f'<div style="min-height:80px;background:rgba(253,248,240,.3);'
            ...
            st.markdown(
                ...
                f'<table style="width:100%;min-width:360px;border-collapse:separate;border-spacing:0">'
